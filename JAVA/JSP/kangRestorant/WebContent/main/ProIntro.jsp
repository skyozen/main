...
<!--------- 위가 top 메뉴 아래부턴 내용------>
<!---------------------->
<%if(request.getAttribute("pro_path")==null){
	
}else{ String name="admin";
%>
<div id="container">
        ...
        <div id="content" class="cboth program_temp">
            ...
            <%String pro_path="";%>
            <%pro_path=(String)request.getAttribute("pro_path") ;%>
            <%if(pro_path.equals(null)){ %>
            <a>등록된 이미지가 없습니다.</a>
            <form action="ProIntroFileUpload.ma" method="post" enctype="multipart/form-data">
            <input name="pro_pic" type="file"/> <input type="submit" value="등록" >
			</form>
      
            <%}%>
            <%
            	if(ME_ID.equals(name)){
            %>
            <form action="ProIntroFileUpload.ma" method="post" enctype="multipart/form-data">
            <input name="pro_pic" type="file"/> <input type="submit" value="등록" >
			</form>
